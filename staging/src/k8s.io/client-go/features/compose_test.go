...
func TestDriveAddFeaturesToExistingFeatureGates(t *testing.T) {
	defaultKubernetesFeatureGates["MyFeature"] = clientgofeaturegates.FeatureSpec{
		Default:       true,
		LockToDefault: true,
		PreRelease:    "GA",
	}
...
func (f *fakeReader) Enabled(key clientgofeaturegates.Feature) bool {
	return true
}

type fakeRegistry struct {
	specs map[clientgofeaturegates.Feature]clientgofeaturegates.FeatureSpec
}

func (f *fakeRegistry) Add(specs map[clientgofeaturegates.Feature]clientgofeaturegates.FeatureSpec) error {
	f.specs = specs
	return nil
}
