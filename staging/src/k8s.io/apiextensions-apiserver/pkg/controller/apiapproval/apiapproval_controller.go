...
// ConditionController is maintaining the KubeAPIApproved condition.
type KubeAPIApprovedConditionController struct {
...
// NewKubeAPIApprovedConditionController constructs a KubeAPIApproved schema condition controller.
func NewKubeAPIApprovedConditionController(
	crdInformer informers.CustomResourceDefinitionInformer,
	crdClient client.CustomResourceDefinitionsGetter,
) *KubeAPIApprovedConditionController {
	c := &KubeAPIApprovedConditionController{
		crdClient:                   crdClient,
		crdLister:                   crdInformer.Lister(),
		crdSynced:                   crdInformer.Informer().HasSynced,
		queue:                       workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "kube_api_approved_condition_controller"),
		lastSeenProtectedAnnotation: map[string]string{},
	}
...
// calculateCondition determines the new KubeAPIApproved condition
func calculateCondition(crd *apiextensions.CustomResourceDefinition) *apiextensions.CustomResourceDefinitionCondition {
	...
	case apihelpers.APIApprovalInvalid:
		return &apiextensions.CustomResourceDefinitionCondition{
			Type:    apiextensions.KubeAPIApproved,
...
func (c *KubeAPIApprovedConditionController) sync(key string) error {
	...
	old := apiextensions.FindCRDCondition(inCustomResourceDefinition, apiextensions.KubeAPIApproved)
...
func (c *KubeAPIApprovedConditionController) Run(threadiness int, stopCh <-chan struct{}) {
	defer utilruntime.HandleCrash()
	defer c.queue.ShutDown()

	klog.Infof("Starting KubeAPIApprovedConditionController")
	defer klog.Infof("Shutting down KubeAPIApprovedConditionController")
...
func (c *KubeAPIApprovedConditionController) runWorker() {
	for c.processNextWorkItem() {
	}
}

// processNextWorkItem deals with one key off the queue.  It returns false when it's time to quit.
func (c *KubeAPIApprovedConditionController) processNextWorkItem() bool {
...
func (c *KubeAPIApprovedConditionController) enqueue(obj *apiextensions.CustomResourceDefinition) {
...
func (c *KubeAPIApprovedConditionController) addCustomResourceDefinition(obj interface{}) {
	castObj := obj.(*apiextensions.CustomResourceDefinition)
	klog.V(4).Infof("Adding %s", castObj.Name)
	c.enqueue(castObj)
}

func (c *KubeAPIApprovedConditionController) updateCustomResourceDefinition(obj, _ interface{}) {
	castObj := obj.(*apiextensions.CustomResourceDefinition)
	klog.V(4).Infof("Updating %s", castObj.Name)
	c.enqueue(castObj)
}

func (c *KubeAPIApprovedConditionController) deleteCustomResourceDefinition(obj interface{}) {
