...
const (
	...
	APIApproved
	// APIUnapproved if the annotation starts with "unapproved" indicating that for whatever reason the API isn't approved, but we should allow its creation
	APIUnapproved
...
func GetAPIApprovalState(annotations map[string]string) (state APIApprovalState, reason string) {
	...
	case strings.HasPrefix(annotation, "unapproved"):
		return APIUnapproved, fmt.Sprintf("not approved: %q", annotation)
